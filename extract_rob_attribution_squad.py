...
def read_annotations():
    ...
    ids = [x[0] for x in fields]
    kwds = [x[1].split(',') for x in fields]
    return OrderedDict(zip(ids, kwds))
...
def aggregated_link_attribution_in_question(tokenizer, feature, attribution, targets):
    target_segments = []
    for tok in targets:
        target_segments.extend(extract_token_segments(tokenizer, feature, tok))
    # print(targets)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_filename",
        type=str,
        default="./features/sample-addsent_squad_roberta-base_512",
    )
    ...
    # sample_id = list(inputs_dicts.keys())[0]
